...
def nodeSubmitApi(cbor_data,url):
    try:
        response = requests.post(url, headers={'Content-Type': 'application/cbor'}, data=cbor_data, timeout=5)
...
while True:
    try:
        ...
        if contract_happy:
            ...
            print('Updating Chain State UTxOs \N{chains}')
            minerUtxos = chain_context.utxos(address)
            print('Updating miners Wallet UTxOs \N{compass}')
        ...
        contract_happy = True
        if SUBMIT_API != '':
            submitApiRes = nodeSubmitApi(signed_tx.to_cbor(),SUBMIT_API)
            print(f'Node submit response: {submitApiRes}')
        sleep_cnt = 0
        while True:
            time.sleep(5)
            txFound = False
